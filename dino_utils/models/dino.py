...
class LayerNorm(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x_norm = self.hook("weighted_mean", ret=x_norm)
        if self.elementwise_affine:
...
class Mlp(nn.Module) : 
    ...
    def forward(
        ...
        x = self.hook('gelu.post', ret=self.act(x))
        x = self.hook('dropout1.post', ret=self.drop(x))
        x = self.hook('c_proj.post', ret=self.fc2(x))
...
class MemEffAttention(Attention):
    def forward(self, x: Tensor, attn_bias=None) -> Tensor:
        if not XFORMERS_AVAILABLE:
            ...
            # I want it to call the same forward in all cases for now
            return super().forward(x)
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.training and self.sample_drop_ratio > 0.1:
            ...
            # Hooking the pre attn, mlp things
            x = self.hook("pre", ret=x) 
            
            # Hooking after applying norm and attention.
            after_norm1 = self.norm1(x)
            after_attn = self.attn(after_norm1)
            after_attn = self.hook("after_attn", ret=self.ls1(after_attn))
            x = x + after_attn
            
            # Hooking after applying norm and mlp
            after_norm2 = self.norm2(x)
            after_mlp = self.mlp(after_norm2)
            after_mlp = self.hook("after_mlp", ret=self.ls2(after_mlp))
            x = x + after_mlp
            
            # Hooking the post results of mlp + attention
...
class DinoVisionTransformer(nn.Module):
    ...
    def forward_features(self, x, masks=None):
        ...
        # Here for ln_pre_post
        x = self.hook("ln_pre_pre", ret=x)
        for blk in self.blocks:
            x = blk(x)
 
        x = self.hook("ln_pre_post", ret=x) # IT works till here
        x_norm = self.norm(x)
        x_norm = self.hook("ln_post_post", ret=x)
        # print(x_norm.shape)
        self.hook.finalize()
        return {
            "x_norm_clstoken": x_norm[:, 0],
